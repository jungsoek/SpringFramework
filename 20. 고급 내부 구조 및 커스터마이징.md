...
**Spring 컨테이너란?**
 → 객체(Bean)의 생성, 관리, 의존성 주입, 생명주기 등을 담당하는 **중앙 관리소**야.
...
→ 실무에서는 거의 항상 `ApplicationContext` 또는 그 하위 구현체를 사용해.
...
Spring이 **"이 클래스는 어떤 Bean으로 등록될 수 있는지에 대한 메타 정보"**를 담고 있는 객체야.
...
`ApplicationContext`가 초기화되기 전,
 `BeanFactory`에 등록된 모든 `BeanDefinition`을 가공할 수 있는 **후처리기**들이 실행돼.
...
생성 및 DI가 완료된 후, Bean은 초기화 단계로 넘어가.
...
- `BeanDefinition`은 **Bean을 정의하기 위한 메타정보**를 담고 있는 객체야.
- 즉, Spring이 **클래스를 실제 Bean으로 등록하기 전의 설계도**라고 생각하면 돼.
...
- 즉, Bean이 실제로 생성되기 전 단계에서, **정의 자체를 가공**할 수 있어.
...
>  애플리케이션 런타임에 이를 **자동으로 탐지하여 로드**할 수 있도록 하는 구조야.

즉, “**구현체는 나중에 주고, 시스템은 알아서 찾아서 쓰게 해줘**”라는 설계 방식이지.
...
Spring Boot에서는 SPI 등록을 **`spring.factories`** 파일을 통해 수행해.
...
> JVM에서 클래스 파일(`.class`)을 읽어와 **메모리(메타 영역)**에 적재하는 **동적 로딩 시스템**이야.
