...
> JDBC는 Java에서 **관계형 데이터베이스(RDB)**와 통신할 수 있도록 해주는 **표준 API(인터페이스)**야.

JDBC는 Java 코드에서 **SQL 실행 → 결과 조회 → DB 연결 관리**를 할 수 있도록 해주는
 **Java SE 표준 라이브러리**이며, **DB 벤더는 JDBC 드라이버를 제공**해야 해.
...
➡️ ORM은 위 과정을 **SQL 없이 객체 단위로 처리**할 수 있도록 해줘.
...
> 📌 Java에서 "ORM"이라 하면 보통 `JPA(Hibernate)`를 의미해.
...
>
> - **JPA는 "규칙서"**,
> - **Hibernate는 "규칙을 따라 만든 실제 프로그램"**이야.
...
- ## `@Entity`, `@Table`, `@Id`, `@GeneratedValue`, `@Column`
  ...
  > JPA에서 이 클래스가 **영속성 관리 대상(엔티티)**임을 나타냄.
...
- ## `JpaRepository`, `CrudRepository`
  ...
  >  **JPA 엔티티에 대한 CRUD, 페이징, 정렬, 쿼리 메서드 기능을 자동으로 제공**해.
...
- ## EntityGraph
  ...
  public class Member { ... }
  java코드 복사@EntityGraph(value = "Member.withTeam")
